...
Generated code: `internal/0/config/schema/schema_tommy.go` is generated by
`//go:generate tommy generate` (the `ManeaterConfig` struct lives in the
isolated `internal/0/config/schema` package so the generator can regenerate it
in place; tommy names its output after the directive's source file). Do not
edit it manually — not even a mechanical import rewrite. Regenerate with
`just codemod-generate` (runs the flake-pinned tommy inside the `maneater-gen`
sandbox) after any tommy input bump; `just verify-generated` (part of
`just verify`) fails when the committed file differs from the pinned output.
